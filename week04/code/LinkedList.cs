...
public class LinkedList : IEnumerable<int>
{
    ...
    {
        // If the list has only one item in it, then set head and tail 
        // to null resulting in an empty list. This also covers an empty list.
        if (_head == _tail)
        {
            _head = null;
            _tail = null;
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                ...
                // Otherwise, it's in the middle, so reconnect the neighbors
                else
                {
                ...
                return; // Stop searching once we find and remove the match
            }
    ...
    {
        // Go through every node in the list, starting at the head.
        Node? curr = _head;
        while (curr is not null)
        {
            if (curr.Data == oldValue)
            {
                curr.Data = newValue; // Update the value in place
            }

            curr = curr.Next; // Keep going, don't stop after the first match
        }
    }
